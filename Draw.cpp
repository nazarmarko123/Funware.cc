...
void CDraw::Init()
{
	...
	g_pSurface->SetFontGlyphSet(font_esp, XorStr("Smallest Pixel-7"), 10, 100, 0, 0, FONTFLAG_OUTLINE);
	...
	g_pSurface->SetFontGlyphSet(font_build, XorStr("Vermin vibes"), 36, FW_EXTRABOLD, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	...
	in_game_logging_font = g_pSurface->SCreateFont();
	g_pSurface->SetFontGlyphSet(in_game_logging_font, XorStr("Protoculture [RUS by Daymarius]"), 20, 500, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);

}

Vector2D CDraw::GetTextSize(unsigned int Font, std::string Input)
{
	/* char -> wchar */
...
void CDraw::DrawF(int X, int Y, unsigned int Font, bool center_width, bool center_height, Color Color, std::string Input)
{
	/* char -> wchar */
	size_t size = Input.size() + 1;
	auto wide_buffer = std::make_unique<wchar_t[]>(size);
	...
	/* call and draw*/
	g_pSurface->DrawSetTextColorxd(Color);
...
void CDraw::polygon_outlined(int count, Vertex_t* Vertexs, Color color, Color colorLine)
{
	static int x[128];
	static int y[128];
