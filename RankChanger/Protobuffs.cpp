...
#include "Changer\profile_info_changer.h"
#include "Changer\inventory_changer.h"
...
{
	if (messageType == k_EMsgGCCStrike15_v2_MatchmakingGC2ClientHello && Clientvariables->RankChanger.enabled)
	{
...
void Protobuffs::EquipAll()
{
	for (auto& x : g_skins) {
		...
		EquipWeapon(def_index, 3, GetSlotID(def_index));
		EquipWeapon(def_index, 2, GetSlotID(def_index));
	}
}