...
void CRender::Init(IDirect3DDevice9* pDevice)
{
	...
	D3DXCreateFont(this->device, 12, 0, FW_BOLD, 1, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, "Tahoma", &this->Fonts.esp);


	D3DXCreateFontA(this->device, this->menu_control_size, 0, 400, 1, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLEARTYPE_QUALITY, DEFAULT_PITCH, "Verdana", &this->Fonts.menu_control);
...
void CRender::Circle(int x, int y, int radius, int points, D3DCOLOR color)
{
	...
	for (int i = 0; i <= points; i++)
		pVertex[i] = { x + radius * cos(D3DX_PI * (i / (points / 2.0f))), y - radius * sin(D3DX_PI * (i / (points / 2.0f))), 0.0f, 1.0f, color };
	this->device->SetFVF(D3DFVF_XYZRHW | D3DFVF_DIFFUSE);
