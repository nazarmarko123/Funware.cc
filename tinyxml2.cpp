...
namespace tinyxml2
{
	...
	static const CBaseEntity entities[NUM_ENTITIES] = {
		{ "quot", 4, DOUBLE_QUOTE },
		{ "amp", 3, '&' },
		{ "apos", 4, SINGLE_QUOTE },
		{ "lt", 2, '<' },
						...
						*q++ = LF;
						...
						if (*(p + 1) == '#') {
							...
							for (int i = 0; i < NUM_ENTITIES; ++i) {
								const CBaseEntity& entity = entities[i];
	...
	*/
		...
		_compactMode(compact)
	{
		for (int i = 0; i<ENTITY_RANGE; ++i) {
			_entityFlag[i] = false;
			_restrictedCBaseEntityFlag[i] = false;
		}
		for (int i = 0; i<NUM_ENTITIES; ++i) {
		...
		_restrictedCBaseEntityFlag[(unsigned char)'&'] = true;
		_restrictedCBaseEntityFlag[(unsigned char)'<'] = true;
		_restrictedCBaseEntityFlag[(unsigned char)'>'] = true;	// not required, but consistency is nice
		_buffer.Push(0);
	}
	...
	void XMLPrinter::PrintString(const char* p, bool restricted)
	{
		...
		if (_processEntities) {
			const bool* flag = restricted ? _restrictedCBaseEntityFlag : _entityFlag;
			while (*q) {
				TIXMLASSERT(p <= q);
