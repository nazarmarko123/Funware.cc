...
public:
	...
	bool GetInterfaces()
	{
		...
		g_pClient = (CClient*)GetPointer(XorStr("client_panorama.dll"), XorStr("VClient"));
		g_pEntitylist = (CClientEntityList*)GetPointer(XorStr("client_panorama.dll"), XorStr("VClientEntityList"));
		...
		g_pGameMovement = (CGameMovement*)GetPointer(XorStr("client_panorama.dll"), XorStr("GameMovement"));
		g_pPrediction = (CPrediction*)GetPointer(XorStr("client_panorama.dll"), XorStr("VClientPrediction"));
		...
		g_pInput = *(CInput**)(FindPatternIDA(XorStr("client_panorama.dll"), XorStr("B9 ? ? ? ? F3 0F 11 04 24 FF 50 10")) + 1);
		g_pGlobals = **(CGlobalVarsBase***)((*(uintptr_t**)g_pClient)[0] + 0x1B);//(CGlobalVarsBase*)*(PDWORD)*(PDWORD)(Utilities::Memory::FindPattern(XorStr("client_panorama.dll"), (PBYTE)XorStr("\xA1\x00\x00\x00\x00\x8B\x40\x10\x89\x41\x04"), XorStr("x????xxxxxx")) + 0x1);
		g_pClientMode = **(CClientModeShared***)((*(uintptr_t**)g_pClient)[10] + 0x5);
		g_pMoveHelper = **(IMoveHelper***)(FindPatternIDA(XorStr("client_panorama.dll"), XorStr("8B 0D ? ? ? ? 8B 46 08 68")) + 0x2);
		g_pViewRenderBeams = *(IViewRenderBeams**)(FindPatternIDA("client_panorama.dll", "B9 ? ? ? ? A1 ? ? ? ? FF 10 A1 ? ? ? ? B9") + 1);		
		g_pFireBullet = (C_TEFireBullets*)((DWORD)GetModuleHandle("client_panorama.dll") + 0xA5B498);//(FindPatternIDA("client_panorama.dll", "8B D1 B8 ? ? ? ? B9") + 0x81);
