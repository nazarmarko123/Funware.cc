...
bool _fastcall hkDoPostScreenSpaceEffects(void* ecx, void* edx, CViewSetup* pSetup)
{
	...
	float Opacity = Clientvariables->Visuals.Opacity;

	float LRed = Clientvariables->Colors.LocalGlow[0];
	float LGreen = Clientvariables->Colors.LocalGlow[1];
	float LBlue = Clientvariables->Colors.LocalGlow[2];
	...
	if (Clientvariables->Visuals.Glow && g_GlowObjManager && g_pEngine->IsConnected()) {
		if (G::LocalPlayer) {
			for (int i = 0; i <= g_GlowObjManager->m_GlowObjectDefinitions.Count(); i++) {
				if (g_GlowObjManager->m_GlowObjectDefinitions[i].IsUnused() || g_GlowObjManager->m_GlowObjectDefinitions[i].getEnt()->IsDormant() || !g_GlowObjManager->m_GlowObjectDefinitions[i].getEnt())
					continue;
				...
				if (glowEnt != nullptr) {
					...
					case 35:
						...
						if (Clientvariables->Visuals.LocalGlow && glowEnt->getEnt() == G::LocalPlayer)
							glowEnt->set(LRed, LGreen, LBlue, Opacity);

						if (Clientvariables->Visuals.EnemyOnly && glowEnt->getEnt()->GetTeamNum() == G::LocalPlayer->GetTeamNum())
							break;

						glowEnt->set(Red, Green, Blue, Opacity);
						break;
					}
				}
			}
