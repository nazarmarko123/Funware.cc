		...
		if (!latest_record.needs_extrapolation) {
			...
			for (auto HitboxID : GetHitboxesToScan(pTarget)) {
				...
				for (int k = 0; k < Points.size(); k++) {
					...
					int hitgroup = -1;
					if (g_pEngineTrace->IsVisible(G::LocalPlayer, vEyePos, vPoint, pTarget, hitgroup)) {
						float modified_damage = wpn_data->damage * (float)pow(wpn_data->range_modifier, wpn_data->range * 0.002);

						ScaleDamage(hitgroup, pTarget, wpn_data->armor_ratio, modified_damage);
