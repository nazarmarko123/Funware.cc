...
ImGuiStyle::ImGuiStyle()
{
	...
	Colors[ImGuiCol_Border] = ImVec4(0.15f, 0.15f, 0.15f, 0.22f);
	Colors[ImGuiCol_BorderShadow] = ImVec4(0.00f, 0.00f, 0.00f, 0.00f);
	Colors[ImGuiCol_FrameBg] = ImVec4(0.15f, 0.15f, 0.15f, 0.30f);   // Background of checkbox, radio button, plot, slider, text input
...
void ImGui::NewFrame()
{
	...
	if (!g.Initialized)
	{
		...
		//IM_ASSERT(g.Settings.empty()); Assertion error
...
void ImGui::RenderCollapseTriangle(ImVec2 p_min, bool is_open, float scale)
{
	...
	const float r = h * 0.30f * scale;
	ImVec2 center = p_min + ImVec2(h*0.60f, h*0.50f*scale);
...
static bool BeginPopupExxd(const char* str_id, ImGuiWindowFlags extra_flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (flags & ImGuiWindowFlags_ShowBorders)
			{
				window->DrawList->AddRect(window->Pos + ImVec2(19, 19), window->Pos + window->Size + ImVec2(19, 19), GetColorU32(ImGuiCol_BorderShadow), window_rounding);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	RenderTextClipped(frame_bb.Min, frame_bb.Max - ImVec2(0, 2), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
...
bool ImGui::SliderInt(const char* label, int* v, int v_min, int v_max, const char* display_format)
{
	...
	float v_f = (float)*v;
	bool value_changed = newSliderFloat(label, &v_f, (float)v_min, (float)v_max, display_format, 1.0f, ImVec2(0, 0), ImVec4(0.09f, 0.26f, 0.55f, 0.90f));
	*v = (int)v_f;
	return value_changed;
}

bool ImGui::SliderMessage(const char* label, int* v, int v_min, int v_max, int at_value_message, const char* value_message, const char* display_format)
{
	if (*v == at_value_message)
		display_format = value_message;
	else
		display_format = "%.0f";
...
bool ImGui::RadioButton(const char* label, bool active)
{
	...
	window->DrawList->AddCircleFilled(center, radius, GetColorU32((held && hovered) ? ImGuiCol_Buttontemp2 : hovered ? ImGuiCol_Buttontemp2 : ImGuiCol_Buttontemp2), 16);
	if (active)
	{
		const float check_sz = ImMin(check_bb.GetWidth(), check_bb.GetHeight());
		const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f));
...
static void
OptionsPopup(ImGuiColorEditFlags flags, float* ref_col)
{
